...
const cities = ["Solo", "Bandung", "Jakarta"];

export default function RecommendationSection() {
  const [city, setCity] = useState("Solo");
  const [start, setStart] = useState(0);

  const filtered = recommendationKost.filter((item) => item.city === city);
      ...
      <div className="mx-auto max-w-6xl px-4">
        ...
        <div className="mb-8 flex items-center justify-between">
          <div className="flex items-center gap-2">
            <h2 className="text-2xl font-semibold">Rekomendasi kos di</h2>
        ...
        <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-4">
          {visible.map((kost) => (
            <RecommendationCard key={kost.id} kost={kost} />
          ))}
        </div>
      </div>
    </section>
